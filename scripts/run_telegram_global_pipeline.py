#!/usr/bin/env python
"""Run the local Telegram gated download pipeline inside XPfilter."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="SQLite database used by XPfilter.",
    )
    parser.add_argument(
        "--checkpoint",
        type=Path,
...
def main() -> int:
    ...
    if not args.skip_download:
        ...
        if flat_links_root is None:
            download_command[
                download_command.index("--flat-links-root") : download_command.index(
                    "--flat-links-root"
                )
                + 2
            ] = []
