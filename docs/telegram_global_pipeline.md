...
- 推理缓存库：`data/xpfilter.db`
- 默认 session 配置：`configs/config.yaml -> telegram.session_name = xpfilter_tg`
...
- 自动使用 `configs/config.yaml` 里的默认 session 名 `xpfilter_tg`。
...
如果做过一次新的交互式登录，只要把新的 session 固化为 `data/tg_session/xpfilter_tg.session`，
...
python scripts/run_tg_gated_download.py \
  --min-score 7.2 \
  --session-name xpfilter_tg_fresh \
